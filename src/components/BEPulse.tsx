...
import { getBEStationsWeather } from "@/app/actions";
...
export default function BEPulse() {
  const [stations, setStations] = useState<PulseStation[]>([]);

  useEffect(() => {
    const load = () => {
      getBEStationsWeather().then((data) => {
        setStations([...data].sort((a, b) => a.name.localeCompare(b.name, "nl-BE")));
      });
    };
    load();
