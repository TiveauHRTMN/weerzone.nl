...
import { getLUStationsWeather } from "@/app/actions";
...
export default function LuxPulse() {
  const [stations, setStations] = useState<PulseStation[]>([]);

  useEffect(() => {
    const load = () => {
      getLUStationsWeather().then((data) => {
        setStations([...data].sort((a, b) => a.name.localeCompare(b.name, "fr-LU")));
      });
    };
    load();
