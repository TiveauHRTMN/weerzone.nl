...
def analyze_and_decide(context):
    prompt = f"""
    Je bent Magnolia, een 3-Headed AI-Agent aangestuurd door Gemini 3 Flash infrastructuur.
    ...
    HUIDIGE STRATEGIE & KAPITAAL (VANAF 6 MEI 2026):
    - Rowano investeert vanaf woensdag 6 mei elke maand €25 in de vloot.
    - TOT 6 MEI: Alleen 'Airdrop Heat' transacties uitvoeren op kleine schaal.
    - VANAF 6 MEI: Protocol 3 (Hunter/Polymarket) en Protocol 2 (Banker) krijgen actief budget.
    - AIRDROP SCHEMA: Verplichte 'Heat' swaps (bijv. 0.005 SOL <-> USDC) op Maandag, Woensdag, en Vrijdag voor de Jupuary 2027 snapshot.
    ...
    JE PROTOCOLLEN:
    1. **The Guardian (SOL/JitoSOL)**: Gebruik Hermes voor staking en Airdrop Heat (Ma-Wo-Vr).
    2. **The Banker (JLP)**: Beheer liquiditeit op Jupiter via Hermes.
    3. **The Hunter (Polymarket - Vitalik Methode)**: Scan via Hermes APIs. Zoek naar 'Anti-Insanity' trades (Pas uitvoeren als budget > €10 beschikbaar is).
    4. **PEV**: Verplichte verificatie na actie.
            ...
            try:
                ...
                if sig:
                    ...
                    if target_found:
                        ...
                        # Hard stop op de huidige executie-thread d.m.v. exception (geschikt voor serverless)
                        print("🛑 PEV Protocol blokkeert verdere acties. Serverless run geaborteerd.", flush=True)
                        raise Exception("PEV FAILED: Geen verificatie na swap.")
            except Exception as e:
...
def run_once():
    """
    SERVERLESS EXECUTIE
    Dit vervangt de lokale 'while True' loop. Wordt nu via Vercel Cron aangeroepen.
    """
    print("\n" + "="*45, flush=True)
    print("--- WEERZONE CRYPTO SYNDICAAT - MAGNOLIA FLASH (SERVERLESS) ---", flush=True)
    print("="*45, flush=True)
    
    context = get_market_context()
    if "error" not in context:
        sol_bal = context.get('sol_balance', 0)
        hermes_logger.log_action("Magnolia", "system_check", f"Serverless Mode online. Balans: {sol_bal:.4f} SOL", status="active")
