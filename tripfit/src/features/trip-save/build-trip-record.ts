...
export interface TripRecord {
  ...
  profile: {
    userId: string;
    travelPartyType: "SOLO" | "COUPLE" | "FAMILY" | "FRIENDS";
    adults: number;
    childAges: number[];
  ...
  preferenceSnapshot: Omit<TripRecord["profile"], "userId">;
}
...
export function buildTripRecord(
  ...
  const travelPartyType = request.travelers.childAges.length > 0
    ? ("FAMILY" as const)
  ...
  const preferenceSnapshot = {
    ...
    childAges: [...request.travelers.childAges],
  ...
  return {
    ...
    travelers,
    interests,
    profile: { userId, ...preferenceSnapshot },
    preferenceSnapshot,
  };
}
