    ...
    async createTrip(record) {
      return prisma.$transaction(async (tx) => {
        ...
        await tx.travelerProfile.upsert({
          where: { userId: record.profile.userId },
          create: record.profile,
          update: {
            travelPartyType: record.profile.travelPartyType,
            adults: record.profile.adults,
            childAges: record.profile.childAges,
            budgetLevel: record.profile.budgetLevel,
            interests: record.profile.interests,
            preferredPace: record.profile.preferredPace,
            mobility: record.profile.mobility,
          },
        });
        await tx.tripPreferenceSnapshot.create({
          data: { ...record.preferenceSnapshot, tripId: trip.id },
        });
