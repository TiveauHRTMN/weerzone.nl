...
describe("buildTripRecord", () => {
  it("maps the main scenario to a complete transactional record", () => {
    ...
    expect(record.preferenceSnapshot).toEqual({
      travelPartyType: "FAMILY",
      adults: 2,
      childAges: [7],
      budgetLevel: "BALANCED",
  ...
  it("infers a solo profile without pretending to know preferences", () => {
    const record = buildTripRecord(
      { ...request, travelers: { adults: 1, childAges: [] } },
      options,
    );
    expect(record.profile.travelPartyType).toBe("SOLO");
    expect(record.profile.budgetLevel).toBe("BALANCED");
  });
