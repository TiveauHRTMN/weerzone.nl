...
Legacy `/mijnweer` and `/waarschuwingen` (plus `/mijn-weer`, `/jouwweer`) are 308-redirected to `/piet` and `/reed` via `next.config.ts` `redirects()`. The persona pages (`/piet`, `/reed`, `/koos`, `/steve`) are the live 200 routes — use those.
...
- `next.config.ts` is **not** empty: it sets a Report-Only CSP, full security header bundle, legacy redirects (`/homepage`, `/mijnweer`, `/waarschuwingen`, `/reisweer/*`), `trailingSlash: false`, `typescript.ignoreBuildErrors: true`, and `experimental.cpus: 1`. Add new headers/rewrites/redirects here, not ad-hoc in routes.
